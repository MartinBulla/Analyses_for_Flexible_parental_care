{# INFO
# R Script generates statistical outputs from
...
#	- datafiles from http://osf.io/bqu3w
	    # for_analyses.Rdata - acquired via download from http://osf.io/xteh4
        ...
        # csv files available from  https://osf.io/t9e2x/
	    # supporting Rdata file from https://osf.io/gfe3v/ (map_for_supplement_10res.Rdata)
#--------------------------------------------------------------------------------------------------------
...
{# RESULTS & Supplementary
  ...
  {# Change in nest attendance
		...
		{# Supplementary Figure 1a, Supplementary Table 3
			...
			{# Supplementary Data Table 3
				...
				# create xlsx		
					...
					{# model assumptions simple
						#dev.new(width=6,height=9)
						png(paste(out_,"model_ass/Supplementary_Table_3.png", sep=""), width=6,height=9,units="in",res=600)
						m=lmer(att~scale(prop_ip)*sex+(prop_ip|act_ID)+(prop_ip|sp),dd, REML=FALSE)	
		  ...
		  {# Supplementary Table 4
				...
				# create xlsx		
					...
					{# model assumptions simple
						...
						png(paste(out_,"model_ass/Supplementary_Table_4.png", sep=""), width=6,height=9,units="in",res=600)
						par(mfrow=c(4,4))
		...
		{# Supplementary Figure 1b-c, Supplementary Table 5
			...
			{# Supplementary Table 5
				...
				{# model assumptions 
						...
						png(paste(out_,"model_ass/Supplementary_Table_5.png", sep=""), width=6,height=9,units="in",res=600)
						par(mfrow=c(4,4))
		...
		{# not in the paper - controlling for temporal autocorrelation - fits remain the same
  ...
  {# Nest success	
		{# run first
				...
				# center within before and after regular incubation period (not used)
		...
		{# Figure 5, Supplementary Table 7 | Effects of start and duration of uniparental incubation on the probability of hatching
			...
			{# quick check whetheer sex could bias the results
			...
			{# Supplementary Table 7 | Effects of start and duration of uniparental incubation on the probability of hatching
				...
				# create xlsx		
					...
					{# model assumptions
						#dev.new(width=6,height=9)
						png(paste(out_,"model_ass/Supplementary_Table_7.png", sep=""), width=6,height=9,units="in",res=600)
						m=glmer(success_bin~scale(prop_ip)+scale(uni_last)+scale(att_med)+(1|sp),data=g3,family='binomial')
						par(mfrow=c(5,3))
						...
						plot(fitted(m), jitter(g3$success_bin, amount=0.05), xlab="Fitted values", ylab="Probability of presence", las=1, cex.lab=1.2, cex=0.8)
							abline(0,1, lty=3)
							t.breaks <- cut(fitted(m), seq(0,1, by=0.1))
							means <- tapply(g3$success_bin, t.breaks, mean)
							semean <- function(x) sd(x)/sqrt(length(x))
							means.se <- tapply(g3$success_bin, t.breaks, semean)
			...
			 
						...
						scatter.smooth(resid(m)~g3$prop_ip);abline(h=0, lty=2, col='red')
						scatter.smooth(resid(m)~g3$uni_last);abline(h=0, lty=2, col='red')
						scatter.smooth(resid(m)~g3$att_med);abline(h=0, lty=2, col='red')
												
						acf(resid(m), type="p", main=list("Temporal autocorrelation:\npartial series residual",cex=0.8))
						
						# spatial autocorrelations - nest location
							spdata=data.frame(resid=resid(m), x=g3$lon, y=g3$lat)
								spdata$col=ifelse(spdata$resid<0,rgb(83,95,124,100, maxColorValue = 255),ifelse(spdata$resid>0,rgb(253,184,19,100, maxColorValue = 255), 'red'))
								#cex_=c(1,2,3,3.5,4)
								cex_=c(1,1.5,2,2.5,3)
								spdata$cex=as.character(cut(abs(spdata$resid), 5, labels=cex_))
								...
								plot(spdata$x, spdata$y,col=spdata$col, cex=as.numeric(spdata$cex), pch= 16, main=list('Spatial dist of resid Barr', cex=0.8),xlim=c(-156.68,-156.6), ylim=c(71.31,71.33))
...
other attached packages:
 [1] multcomp_1.4-5      TH.data_1.0-7       survival_2.39-4    
 [4] mvtnorm_1.0-5       effects_3.1-1       arm_1.8-6          
 [7] lme4_1.1-12         Matrix_1.2-6        MASS_7.3-45        
[10] AICcmodavg_2.0-4    zoo_1.7-13          XLConnect_0.2-11   
[13] XLConnectJars_0.2-9 RSQLite_1.0.0       DBI_0.4-1          
[16] rgdal_1.1-10        rgeos_0.3-19        raster_2.5-8       
[19] maptools_0.8-39     sp_1.2-3            matrixStats_0.50.2 
[22] magrittr_1.5        latticeExtra_0.6-28 RColorBrewer_1.1-2 
[25] lattice_0.20-33     plyr_1.8.3          gridExtra_2.2.1    
[28] ggthemes_3.0.3      ggplot2_2.1.0      

loaded via a namespace (and not attached):
 [1] Rcpp_0.12.5      nloptr_1.0.4     tools_3.3.0      digest_0.6.9    
 [5] gtable_0.2.0     nlme_3.1-128     coda_0.18-1      rJava_0.9-8     
 [9] nnet_7.3-12      stats4_3.3.0     reshape_0.8.5    VGAM_1.0-2      
[13] foreign_0.8-66   minqa_1.2.4      codetools_0.2-14 scales_0.4.0    
[17] splines_3.3.0    abind_1.4-3      assertthat_0.1   unmarked_0.11-0 
[21] colorspace_1.2-6 xtable_1.8-2     labeling_0.3     sandwich_2.3-4  
[25] munsell_0.4.3   
}
