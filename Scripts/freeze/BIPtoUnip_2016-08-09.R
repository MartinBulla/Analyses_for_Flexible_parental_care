...
{# TOOLS
   ...
   {# define functions
	  ...
	  RFID.temperature_actogram = function(dfr,figCap = figCap_,latlon = latlon_, type = "PNG", min_=-3, max_=49, UTC=FALSE, UTC_met=FALSE, signal=FALSE) {
			...
			{# end state and hatching start
				...
				ie_=nbi[nbi$state!='hs' & !is.na(nbi$state),]
					if(nrow(ie_)>0){ie_$day = as.Date(trunc(ie_$datetime_, "day"))
								   ...
								   ie_$act=max_
								   ie_$who=ifelse(ie_$state=="w","warm", ifelse(ie_$state=="d", "deserted", ifelse(ie_$state=="u", "unknown", ifelse(ie_$state=="p", "depredated", ifelse(ie_$state=="hg", "hatching",ifelse(ie_$state=="ln", "out of nest", ifelse(ie_$state=="hd","hatched", NA)))))))
				   				   }
			 }
...
{# prepare database
	...
	{#PESA # careful some nests are in both years
		...
		{# nests to extract the data for
			...
			nests$end=as.POSIXct(nests$end)
			nests$start=as.POSIXct(nests$start)
			nests=nests[!nests$sp%in%c('pesa'),]
			nests_=nests[!duplicated(nests$nest),]
			}
		{# end states
				  n=readWorksheetFromFile(paste(wd,'nests.xls', sep=""), sheet='nest_states',colTypes = c(
		...
		{# extract and plot
			...
			#for (i in c(25, 55,65)) {
				...
				yr= nests_$year[i]
				tag= nests_$tag_ID[i]
				bird_ID=nests_$bird_ID[i]
				print(paste(tag_ID, i,sep=" "))
				a=dbq(con,paste("SELECT tag_ID as tag, CV as cv, datetime_ from loggersatbarrow.cv_inttags where tagType=1 and tag_ID='",tag,"'", sep=""))
				# define incubation
					a$inc=ifelse(a$cv<tr_,1,0) # 1 is incubation
				# uniparental period used in the data	
			 ...
			 {# add variables
					...
					a$who='f'
					a$sys='uniparental' # breeding system
					a$act_ID=paste(a$sys,i+9,sep="_") 
					a$t_method='radio'
					a$disturb=NA
					a$bird_ID=bird_ID
					a$datetime_z=a$datetime_
			 ...
			 {# plot
					...
					# generate actograms
						RFID.temperature_actogram(dfr=dfr_,type="PNG", UTC=TRUE, UTC_met=FALSE,min_=-.05, max_=0.22, signal=TRUE) #type="SAVE")# 		
