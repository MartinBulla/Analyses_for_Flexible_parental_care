...
# add script for lines for unip start and inc end
 ...
 # ISSUES
	...
	# dislocated temperature probe - no use for constancy
		c('s704','s804')
		'w504' # only from jul-09 visit ok
}
...
{# TOOLS
   ...
   {# define functions
	  ...
	  RFID.temperature_actogram = function(dfr,figCap = figCap_,latlon = latlon_, inp=ip_, type = "PNG", min_=-3, max_=49, UTC=FALSE, UTC_met=FALSE, signal=FALSE, day=TRUE) {
		 ...
		 #      tf = paste0(outdir,'/',dfr$nest[1], ".pdf")
		  #     pdf(tf, paper = "a4", width = 8, height = 11.6929134)
			...
			if(UTC==FALSE){dfr$datetime_=dfr$datetime_z}
			 ...
			 sl2$day_j=as.numeric(format(sl2$day ,"%j")) - as.numeric(format(as.Date(trunc(inc_start, "day")),"%j"))+1
			...
			{# end state and hatching start
				...
				ie_=nbi[nbi$state!='hs' & !is.na(nbi$state),]
					if(nrow(ie_)>0){ie_$day = as.Date(trunc(ie_$datetime_, "day"))
								   ...
								   ie_$act=max_
								   ie_$who=ifelse(ie_$state=="w","warm", ifelse(ie_$state=="d", "deserted", ifelse(ie_$state=="u", "unknown", ifelse(ie_$state=="p", "depredated", ifelse(ie_$state=="hg", "hatching",ifelse(ie_$state=="ln", "out of nest", ifelse(ie_$state=="hd","hatched",ifelse(ie_$state=="tag_loss","tag loss", NA))))))))
				   				   }
			 }
			...
			if(type %in% c('SAVE')) {
					...
					tf = paste0(outdir,pkk$act_ID[i], paste("_",pkk$pkk[i],sep=""), "_%03d.png",sep="") 
									...
									tf = paste0(outdir,figCap$ID, paste("_",figCap$year,"_",figCap$nest,sep=""), "_%03d.png",sep="") 
