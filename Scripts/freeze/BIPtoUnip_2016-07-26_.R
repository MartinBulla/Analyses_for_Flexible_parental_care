...
{# prepare database
	...
	{# Barrow data
		...
		{# actogram function BARROW
				RFID.temperature_actogram = function(dfr,figCap = figCap_,latlon = latlon_, type = "PNG", min_=0, max_=50, UTC=FALSE) {
		 ...
		 #      tf = paste0(outdir,'/',dfr$nest[1], ".pdf")
		  #     pdf(tf, paper = "a4", width = 8, height = 11.6929134)
			...
			{# legend 
					{# caption
					clr_0=list(text=list(c(figCap$scinam, figCap$species,figCap$ID),cex=0.6, col=c(wr_col), font=c(3,2,2)),
								points=list(pch=c(15),cex= c(0.8), col=c(tra)))
					clr_e=list(text=list(c(figCap$species,figCap$scinam,  figCap$ID),cex=0.6, col=c(tra), font=c(3,2,2)))
					}
					{# used	for analyses		
						
						r=length(dfr$type[dfr$type=="bip"])
						if(r>0){clr_1=list(	text= list(c(paste('used for uniparental'),paste("used for biparental"), "used for none"),cex=0.6, col=c(wr_col)),
											points=list(pch=c('-','-','-'),cex= c(0.8), col=c(uni_col,bip_col, none_col)))
						
							}else{clr_1=list(	text= list(c(paste('used for uniparental'),paste("used for biparental"),"used for none"),cex=0.6, col=c(wr_col,wr_col,wr_col_out)),
												points=list(pch=c(15,15),cex= c(0.8), col=c(uni_col,bip_col,tra)))
											}
					
						#text = list(c("-","-","|","|"), cex=c(1,1,0.6,0.6), font=c("plain","plain","bold","bold"), col = col_r_p
					}
					{# create legend column for temperatures if present
						clr_3=list(text = list(c("nest temperature incubation","nest temperature no incubation","surface temperature"),cex=0.6, col=wr_col),
														#lines = list(col=act_c$cols[act_c$who%in%c("nest temperature","surface temperature")],lwd=2,size=1))}								
														points = list(col=act_c$cols[act_c$who%in%c("nest temperature incubation","nest temperature no incubation","surface temperature")],cex=0.3))								
					}				
					...
					{# create legend column for f,m, disturb if present
						...
						if(dfr$sp[!is.na(dfr$sp)][1]=="SNPL") {clr_2=list( text = list(c('\u2640 RFID reading','\u2642 RFID reading','   error',''),col=col_r_t,cex=0.6),
												...
												clr_2=list( text = list(c('\u2640 RFID reading','\u2642 RFID reading','   disturbance',''),col=col_r_t,cex=0.6),
												text = list(c("|","|","|","|"), cex=c(0.6,0.6,0.6), font="bold", col = col_r_p))
												}
							}									
					{# sun elevation
						...
						if(nt1==0 & nt2 ==0){	clr_4=list(text = list(c("twilight","night"),col=wr_col_out, cex=0.6),
														   points = list(col=rgb(1,1,1,1,maxColorValue = 1),pch=c(15),cex= c(0.6)))}
						if(nt1>0 & nt2 ==0){	clr_4=list(text = list(c("twilight","night"),cex=0.6, col=c(wr_col[1],wr_col_out)),
														   points = list(col=c(act_c$cols[act_c$who%in%c("sun < 0°")],rgb(1,1,1,1,maxColorValue = 1)),pch=c(15),cex= c(0.6)))}			
						if(nt1==0 & nt2 >0){	clr_4=list(text = list(c("twilight","night"),cex=0.6, col=c(wr_col_out,wr_col)),
														   points = list(col=c(rgb(1,1,1,1,maxColorValue = 1),act_c$cols[act_c$who%in%c("sun < -6°")]),pch=c(15),cex= c(0.6)))}	
						if(nt1>0 & nt2 >0){	clr_4=list(text = list(c("twilight","night"),cex=0.6, col=wr_col),
													   points = list(col=act_c$cols[act_c$who%in%c("sun < 0°","sun < -6°")],pch=c(15),cex= c(0.6)))}
					}
					{# hatch start and end state
						...
						if(in1>0 & in2 >0){	col_i_t=wr_col
											col_i_p=act_c$cols[act_c$who%in%c("hatching start","incubation end")]}	
						
						clr_5=list( text = list(c("hatching start",paste("end:",ie_$who, sep=" ")),cex=0.6, col=col_i_t),
									text = list(c("|","|"),col=col_i_p,cex= c(0.6),font="bold"))
						}							   
			   ...
			   panel1 = function(...) {
							  ...
							  # panel.abline(v=c(6,12,18,24),col = "grey70")
							   ...
							   {#rfid and metadata
									...
									panel.xyplot(dfri$time, dfri$act, col = dfri$cols, type = "h")
		   ...
		   for (i in (1:nrow(nests_))) {
			#for (i in c(25, 55,65)) {
				...
				{# define uniparental and biparental periods, no = not used
				   ...
				   if(!nests_$one_since[i]%in%c('unknown','always')){
									a$type=ifelse(a$datetime_>=nests_$on[nests_$nest==nest & nests_$year==yr]+60*60*2 & a$datetime_<nests_$one_since[nests_$nest==nest & nests_$year==yr],'bip', # biparental incubation from 2 hours after equipment was placed until a parent deserted (after catching or naturally)
												ifelse(a$datetime_>=nests_$start[nests_$nest==nest & nests_$year==yr] & a$datetime_<nests_$end[nests_$nest==nest & nests_$year==yr],'uni', # uniparental incubation from the time the widowed bird arrived to the nest until the end of incubation
												'no'))			
													}
				   if(length(nests$cirumstances[nests$nest==nest & nests$circumstances=='temporal'])==1){ # for nests witho temporal desertion present 
										a$type=ifelse(a$datetime_>=nests_$start[nests_$nest==nest & nests_$year==yr] & a$datetime_<nests_$end[nests_$nest==nest & nests_$year==yr],'uni',a$type) 
										}
				}
				...
				{# add variables
					...
					a$datetime_=  as.character(a$datetime_z+ (nests_$local_plus[i]*60*60)) # bring to longitudanal time
				...
				print(paste(nest, i,sep=" "))
							...
							# get raw incubation data and prepare them for plotting
								...
								# generate actograms
									RFID.temperature_actogram(dfr=dfr_,type="PNG") #type="SAVE")# 
								
								print(pkk[i,])
								print(paste(max(c(dfr$t_surface),na.rm=TRUE), min(dfr$t_surface,na.rm=TRUE)))
								print(paste(max(c(dfr$t_nest_tt,dfr$t_nest_msr),na.rm=TRUE), min(c(dfr$t_nest_tt,dfr$t_nest_msr),na.rm=TRUE)))
