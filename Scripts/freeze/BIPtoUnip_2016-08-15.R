...
# add script for lines for unip start and inc end
 ...
 # ISSUES
	...
	# no use for contancy:
		...
		's815' #no use for constancy (incubate one egg, funny rhythm)	
	# one egg incubation	
		's310','s815'
# have not met UNIPARENTAL definition
		  ...
		  #     pdf(tf, paper = "a4", width = 8, height = 11.6929134)
			...
			{# legend 
					...
					if(nest%in%c('s409','s510','s516')){
						clr_0=list(text=list(c(figCap$scinam, figCap$species,figCap$ID,"Bird removal experiment"),cex=0.6, col=c(wr_col,wr_col,wr_col,wr_col), font=c(3,2,1,2)),
								points=list(pch=c(15),cex= c(0.8), col=c(tra)))
						
						}else if (nest%in%c('s704')){
						clr_0=list(text=list(c(figCap$scinam, figCap$species,figCap$ID,"Dislocated T probe"),cex=0.6, col=c(wr_col,wr_col,wr_col,wr_col), font=c(3,2,1,2)),
								points=list(pch=c(15),cex= c(0.8), col=c(tra)))
						}else if (nest%in%c('s310','s815')){
						clr_0=list(text=list(c(figCap$scinam, figCap$species,figCap$ID,"Incubates one egg"),cex=0.6, col=c(wr_col,wr_col,wr_col,wr_col), font=c(3,2,1,2)),
								points=list(pch=c(15),cex= c(0.8), col=c(tra)))
						}else if(nest%in%c('l301','l401','l501','s304','s902','w504')){
						clr_0=list(text=list(c(figCap$scinam, figCap$species,figCap$ID,"Considered"),cex=0.6, col=c(wr_col,wr_col,wr_col,wr_col), font=c(3,2,1,2)),
								points=list(pch=c(15),cex= c(0.8), col=c(tra)))
						}else{		
