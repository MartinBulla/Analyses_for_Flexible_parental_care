...
{# prepare database
	...
	{# biparental species
		...
		{# load metadata
   		...
   		{# extract and plot
		   for (i in 1:nrow(nests_)) {# 76:77) { #(1:nrow(nests_))) {#
			#for (i in c(25, 55,65)) {
				nest=nests_$nest[i]
				yr= nests_$year[i]
				...
				{# define uniparental and biparental periods, no = not used
				   ...
				   if(length(nests$cirumstances[which(nests$nest==nest & nests$circumstances=='temporal')])==1){ # for nests witho temporal desertion present 
